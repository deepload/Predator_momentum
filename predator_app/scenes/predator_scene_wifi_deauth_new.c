...
static void wifi_deauth_view_draw_callback(Canvas* canvas, void* context) {
    ...
    // Get view state
    WifiDeauthView* state = view_get_model(app->view_dispatcher->current_view);
    if(!state) return;
...
static void wifi_deauth_view_free(View* view) {
    view_free(view);
}
...
void predator_scene_wifi_deauth_new_on_enter(void* context) {
    ...
    // Create custom view
    View* view = wifi_deauth_view_alloc(app);
    ...
    // Start deauth attack if hardware is available
    if(app->esp32_uart) {
        ...
        WifiDeauthView* state = view_get_model(view);
        if(state) {
