...
static void subghz_attacks_view_draw_callback(Canvas* canvas, void* context) {
    ...
    // Get view state
    SubGhzAttackView* state = view_get_model(app->view_dispatcher->current_view);
    if(!state) return;
    ...
    if(state->frequency >= 1000000) {
        snprintf(freq_text, sizeof(freq_text), "Freq: %.1f MHz", state->frequency / 1000000.0f);
    } else {
        snprintf(freq_text, sizeof(freq_text), "Freq: %.1f kHz", state->frequency / 1000.0f);
    }
