...
// #include "predator_ai_optimizer.h"  // DISABLED - Memory optimization
// #include "predator_attack_predictor.h"  // DISABLED - Memory optimization
#include <furi.h>
#include <furi_hal.h>
...
bool predator_car_attack_rolling_code(PredatorApp* app, size_t model_index) {
    ...
    FURI_LOG_I("CarAttack", "🚗 Rolling Code Attack: %s %s", model->make, model->model);
    
    // Use model's frequency (memory-optimized)
    uint32_t frequency = model->frequency;
    
    FURI_LOG_I("CarAttack", "🚗 Using frequency: %lu Hz", frequency);
